...
coerciveness_results <- data.frame(Question = paste0("Q3 - Coerciveness_", 1:18),
                                   Statement = rep(NA, 18), Mean = NA, SD = NA, N = NA)

true_confession_results <- data.frame(Question = paste0("Q4 - True Confession_", 1:18),
                                      Statement = rep(NA, 18), Mean = NA, SD = NA, N = NA)

false_confession_results <- data.frame(Question = paste0("Q5 False Confess._", 1:18),
                                       Statement = rep(NA, 18), Mean = NA, SD = NA, N = NA)

# Statements for all tactics
leo_liu_statements <- c(
  "Repeatedly accusing a suspect.",
  "Asking the suspect to take a lie detector test.",
  "Repeatedly telling the suspect his or her alibi is false.",
...
# Helper function to fill a results data frame
fill_results <- function(df, question_prefix, statements, data) {
  for (i in 1:18) {
    col_name <- paste0(question_prefix, i)
    df$Statement[i] <- statements[i]
    if (col_name %in% colnames(data)) {
      values <- extract_numeric(data[[col_name]])
      values <- values[!is.na(values)]
      df$Mean[i] <- ifelse(length(values) > 0, round(mean(values), 2), NA)
      df$SD[i]   <- ifelse(length(values) > 0, round(sd(values), 2), NA)
      df$N[i]    <- length(values)
    }
  }
  return(df)
}
...
coerciveness_results <- fill_results(coerciveness_results, "Q3 - Coerciveness_", leo_liu_statements, FC_Panel_Data)
true_confession_results <- fill_results(true_confession_results, "Q4 - True Confession_", leo_liu_statements, FC_Panel_Data)
false_confession_results <- fill_results(false_confession_results, "Q5 False Confess._", leo_liu_statements, FC_Panel_Data)

# Create grouped table (optional)
grouped_by_statement <- data.frame(
  Statement = leo_liu_statements,
  Coerciveness_Mean = coerciveness_results$Mean,
  Coerciveness_SD = coerciveness_results$SD,
...
# Scale notes
coerciveness_note <- "Scale: 1 = Not coercive at all; 5 = Extremely coercive"
true_confession_note <- "Scale: 1 = Not at all likely; 5 = Extremely likely"
false_confession_note <- "Scale: 1 = Not at all likely; 5 = Extremely likely"

# Function to create APA table
create_apa_flextable <- function(data, caption, footer_note, filename) {
  ft <- flextable(data) %>%
    set_header_labels(
      Statement = "Interrogation Tactic",
      Mean = "M", SD = "SD", N = "n"
    ) %>%
    italic(j = c("M", "SD", "n"), part = "header") %>%
    align(j = 1, align = "left", part = "all") %>%
    align(j = 2:4, align = "right", part = "all") %>%
...
# Create and save each table
create_apa_flextable(
  coerciveness_results %>%
    select(Statement, Mean, SD, N) %>%
    rename(M = Mean, n = N),
  caption = "Table 1. Perceived Coerciveness of Interrogation Tactics",
  footer_note = coerciveness_note,
  filename = "leo_liu_coerciveness_apa_table.docx"
)
...
create_apa_flextable(
  true_confession_results %>%
    select(Statement, Mean, SD, N) %>%
    rename(M = Mean, n = N),
  caption = "Table 2. Perceived Likelihood of Techniques Leading to True Confessions",
  footer_note = true_confession_note,
  filename = "leo_liu_true_confession_apa_table.docx"
)
...
false_confess_vars <- c(
  "Q5 - False Confess._1", "Q5 - False Confess._2", "Q5 - False Confess._3",
  "Q5 - False Confess._4", "Q5 - False Confess._5", "Q5 - False Confess._6",
  "Q5 - False Confess._7", "Q5 - False Confess._8", "Q5 - False Confess._9",
  "Q5 - False Confess._10", "Q5 - False Confess._11", "Q5 - False Confess._12",
  "Q5 - False Confess._13", "Q5 - False Confess._14", "Q5 - False Confess._15",
  "Q5 - False Confess._16", "Q5 - False Confess._17", "Q5 - False Confess._18"
)

# Select and clean the false confession columns
false_confess_data <- FC_Panel_Data %>%
  select(all_of(false_confess_vars)) %>%
  mutate(across(everything(), ~ as.numeric(str_extract(., "\\d+(\\.\\d+)?"))))

# Pivot longer and summarize
false_confession_results <- false_confess_data %>%
...
# Suppose your false_confession_results has columns: Statement, Mean, SD, N
# Rename Mean → M, N → n for compatibility
false_confession_results_fixed <- false_confession_results %>%
  rename(
    M = Mean,
    n = N
  )

# Replace the Statement column in the false confession results
false_confession_results_fixed$Statement <- leo_liu_statements

false_confession_results_fixed <- false_confession_results_fixed %>%
  mutate(
    M = round(M, 2),
    SD = round(SD, 2)
  )

# Then create the APA table
create_apa_flextable(
  false_confession_results_fixed %>% select(Statement, M, SD, n),
  caption = "Table 3. Perceived Likelihood of Techniques Leading to False Confessions",
  footer_note = false_confession_note,
  filename = "leo_liu_false_confession_apa_table.docx"
)
```
