...
write_file <- function(x) {cat(x, file = "Kassin_Student.html")}
...
write.csv(all_results[, c("Statement", "Mean", "SD", "N")], "Kassin_Student.csv", row.names = FALSE)
...
if(require(flextable)) {
  ...
  leo_table_data <- data.frame(
    Tactic = leo_liu_statements,
    Coerciveness = paste0(round(coerciveness_results$Mean, 2), " (", round(coerciveness_results$SD, 2), ")"),
    TrueConfession = paste0(round(true_confession_results$Mean, 2), " (", round(true_confession_results$SD, 2), ")"),
    FalseConfession = paste0(round(false_confession_results$Mean, 2), " (", round(false_confession_results$SD, 2), ")")
  )
  ...
  leo_ft <- flextable(leo_table_data) %>%
    ...
    set_caption("Table X. Mean scores and standard deviations for interrogation tactics on coerciveness and likelihood of eliciting true or false confessions.") %>%
    font(fontname = "Times New Roman", part = "all") %>%
    fontsize(size = 12, part = "all") %>%
    line_spacing(space = 1.5) %>%
    border_remove() %>%
    border(i = 1:nrow(leo_table_data), border.top = fp_border(width = 0.5), border.bottom = fp_border(width = 0.5), part = "body") %>%
    hline_top(part = "header", border = fp_border(width = 0.5)) %>%
    hline_bottom(part = "header", border = fp_border(width = 0.5)) %>%
    padding(padding = 5, part = "all") %>%
    autofit()
  
  # Optionally add superscript notation for scale references on first row cells of each numeric column
  leo_ft <- leo_ft %>%
    compose(
      i = 1, j = 2,
      value = as_paragraph(leo_table_data$Coerciveness[1], as_sup("a"))
  ...
  # Save combined table as Word doc
  save_as_docx(leo_ft, path = "leo_interrogation_tactics_combined_table.docx")
}

# Save all individual tables plus combined to one Word document
if(require(flextable) && require(officer)) {
  doc <- read_docx()
  
  # Assuming coerciveness_ft, true_confession_ft, false_confession_ft already exist as flextables
  doc <- body_add_par(doc, "Table 1: Perceived Coerciveness", style = "heading 2")
  doc <- body_add_flextable(doc, coerciveness_ft)
  doc <- body_add_par(doc, "")
  
  doc <- body_add_par(doc, "Table 2: Likelihood of True Confession", style = "heading 2")
  doc <- body_add_flextable(doc, true_confession_ft)
  doc <- body_add_par(doc, "")
  
  doc <- body_add_par(doc, "Table 3: Likelihood of False Confession", style = "heading 2")
  doc <- body_add_flextable(doc, false_confession_ft)
  doc <- body_add_par(doc, "")
  
  doc <- body_add_par(doc, "Table 4: Combined Comparison Table", style = "heading 2")
  doc <- body_add_flextable(doc, leo_ft)
...
This code will combine the student and panel dataset so I can better compare it to what Marc did in 2017

```{r}
#| label: Packages and Data Combination

library(readxl)
library(dplyr)

students <- read_csv("FC SMU Students.csv")
panel <- read_csv("FC Panel Data.csv")

View(students)
students <- students[-c(1:2), -c(1:23)]

panel <- panel[-c(1:2), -c(1:19)]
View(panel)

names(students) <- trimws(names(students))
names(panel) <- trimws(names(panel))

FC_Combined <- bind_rows(students, panel)
View(FC_Combined)

```

```{r}
#| label: Kassin COMBINED Data Tables

# R Script to create a table of means and standard deviations 
# for Kassin et al. (2018) questions from FC_Combined dataset
...
for(i in 1:19) {
  col_name <- paste0("Q1_", i)
  if(col_name %in% colnames(FC_Combined)) {
    numeric_values <- extract_numeric(FC_Combined[[col_name]])
    numeric_values <- numeric_values[!is.na(numeric_values)]
    if(length(numeric_values) > 0) {
      q1_results$Mean[i] <- round(mean(numeric_values, na.rm = TRUE), 2)
      q1_results$SD[i] <- round(sd(numeric_values, na.rm = TRUE), 2)
...
if(require(kableExtra)) {
  ...
  write_file <- function(x) {cat(x, file = "KASSIN_COMBINED.html")}
  write_file(as.character(apa_styled_table))
  
  print(apa_styled_table)
}

write.csv(all_results[, c("Statement", "Mean", "SD", "N")], "Kassin_COMBINED.csv", row.names = FALSE)


```

```{r}
#| label: Leo & Liu COMBINED Data

# R Script to create APA-formatted tables for Leo's Interrogation Tactics data
# using FC_Combined dataset (Q3 - Coerciveness, Q4 - True Confession, Q5 - False Confession)
...
# Function to extract numeric from possibly non-numeric columns
extract_numeric <- function(x) {
  if (is.numeric(x)) return(x)
  if (is.factor(x) || is.character(x)) {
    nums <- as.numeric(gsub("([0-9]+).*", "\\1", x))
...
for (i in 1:18) {
  ...
  if (col_name %in% colnames(FC_Combined)) {
    numeric_values <- extract_numeric(FC_Combined[[col_name]])
    numeric_values <- numeric_values[!is.na(numeric_values)]
    coerciveness_results$Mean[i] <- round(mean(numeric_values), 2)
    coerciveness_results$SD[i] <- round(sd(numeric_values), 2)
    coerciveness_results$N[i] <- length(numeric_values)
  }
}
...
for (i in 1:18) {
  ...
  if (col_name %in% colnames(FC_Combined)) {
    numeric_values <- extract_numeric(FC_Combined[[col_name]])
    numeric_values <- numeric_values[!is.na(numeric_values)]
    true_confession_results$Mean[i] <- round(mean(numeric_values), 2)
    true_confession_results$SD[i] <- round(sd(numeric_values), 2)
    true_confession_results$N[i] <- length(numeric_values)
  }
}
...
for (i in 1:18) {
  ...
  if (col_name %in% colnames(FC_Combined)) {
    numeric_values <- extract_numeric(FC_Combined[[col_name]])
    numeric_values <- numeric_values[!is.na(numeric_values)]
    false_confession_results$Mean[i] <- round(mean(numeric_values), 2)
    false_confession_results$SD[i] <- round(sd(numeric_values), 2)
    false_confession_results$N[i] <- length(numeric_values)
  }
}

# Combine all results
all_leo_liu_results <- bind_rows(
  coerciveness_results,
  true_confession_results,
  false_confession_results
) %>%
  mutate(Category = rep(c("Coerciveness", "True Confession", "False Confession"), each = 18))
...
# Create and export APA-style tables
doc <- read_docx()
doc <- body_add_par(doc, "Table 1: Perceived Coerciveness of Leo's Interrogation Tactics", style = "heading 2")
doc <- body_add_flextable(doc, flextable(coerciveness_results %>% select(Statement, Mean, SD, N)) %>%
                            set_header_labels(Statement = "Interrogation Tactic", Mean = "M", SD = "SD", N = "n") %>%
...
doc <- body_add_par(doc, "")
doc <- body_add_par(doc, "Table 2: Likelihood of True Confession", style = "heading 2")
doc <- body_add_flextable(doc, flextable(true_confession_results %>% select(Statement, Mean, SD, N)) %>%
                            set_header_labels(Statement = "Interrogation Tactic", Mean = "M", SD = "SD", N = "n") %>%
                            italic(j = 2:4, part = "header") %>%
                            align(j = 1, align = "left") %>%
                            autofit() %>%
...
doc <- body_add_par(doc, "")
doc <- body_add_par(doc, "Table 3: Likelihood of False Confession", style = "heading 2")
doc <- body_add_flextable(doc, flextable(false_confession_results %>% select(Statement, Mean, SD, N)) %>%
                            set_header_labels(Statement = "Interrogation Tactic", Mean = "M", SD = "SD", N = "n") %>%
                            italic(j = 2:4, part = "header") %>%
                            align(j = 1, align = "left") %>%
                            autofit() %>%
                            add_footer_lines(false_confession_scale_note))
print(doc, target = "Leo_Interrogation_Tactics_COMBINED_APA_Tables.docx")
...
leo_ft <- flextable(leo_table_data) %>%
  ...
  add_header_row(values = c("", "Mean (SD)", "Mean (SD)", "Mean (SD)"), colwidths = c(1, 1, 1, 1)) %>%
  compose(i = 1, j = 2, value = as_paragraph(leo_table_data$Coerciveness[1], as_sup("a"))) %>%
  compose(i = 1, j = 3, value = as_paragraph(leo_table_data$TrueConfession[1], as_sup("b"))) %>%
  compose(i = 1, j = 4, value = as_paragraph(leo_table_data$FalseConfession[1], as_sup("c"))) %>%
  font(fontname = "Times New Roman", part = "all") %>%
  fontsize(size = 12, part = "all") %>%
  line_spacing(space = 1.5) %>%
  border_remove() %>%
  ...
  autofit() %>%
  set_caption("Table X. Mean scores and standard deviations for interrogation tactics on coerciveness and likelihood of eliciting true or false confessions.") %>%
  add_footer_lines(c(coerciveness_scale_note, true_confession_scale_note, false_confession_scale_note))

print(leo_ft)
save_as_docx(leo_ft, path = "leo_interrogation_tactics_COMBINED_combined_table.docx")

# Save all tables to one doc
doc <- read_docx()
doc <- body_add_par(doc, "Table 1: Perceived Coerciveness", style = "heading 2")
doc <- body_add_flextable(doc, flextable(coerciveness_results %>% select(Statement, Mean, SD, N)))
doc <- body_add_par(doc, "Table 2: Likelihood of True Confession", style = "heading 2")
...
doc <- body_add_par(doc, "Table 4: Combined Comparison Table", style = "heading 2")
doc <- body_add_flextable(doc, leo_ft)
print(doc, target = "leo_interrogation_tactics_COMBINED_all_tables.docx")
...
This is the code I will use to compare the student and panel data between the 2017 and 2024 data. I will also compare the combined dataset from 2017 to 2014 data. 
